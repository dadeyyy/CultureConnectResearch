  ...
  const handleEventClick = useCallback(
    async (clickInfo: EventClickArg) => {
      try {
        ...
        const response = await fetch(`${server}/get-event/${clickInfo.event.id}`, {
          credentials: "include",
        });
        ...
        if (data) {
          ...
          setCurrentEvents(
            data.calendars.map((event: IEvent) => ({
              ...
              repeat: event.repeat,
              startStr: new Date(event.startDate).toISOString().replace(/T.*$/, ""),
            ...
            {!calendarDetails ? (
              ...
              <div className="flex flex-col gap-3 ">
                ...
                <div className="flex flex-col pl-3 h-full">
                  ...
                  {calendarDetails.location?.coordinates ? (
                    <ReactMapGl
